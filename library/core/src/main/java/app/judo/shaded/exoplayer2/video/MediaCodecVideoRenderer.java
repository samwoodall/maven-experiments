 ...
 */
package app.judo.shaded.exoplayer2.video;
...
import app.judo.shaded.exoplayer2.FormatHolder;
...
import app.judo.shaded.exoplayer2.mediacodec.MediaCodecAdapter;
import app.judo.shaded.exoplayer2.mediacodec.MediaCodecDecoderException;
import app.judo.shaded.exoplayer2.mediacodec.MediaCodecInfo;
import app.judo.shaded.exoplayer2.mediacodec.MediaCodecRenderer;
import app.judo.shaded.exoplayer2.mediacodec.MediaCodecSelector;
import app.judo.shaded.exoplayer2.mediacodec.MediaCodecUtil;
import app.judo.shaded.exoplayer2.mediacodec.MediaFormatUtil;
...
import app.judo.shaded.exoplayer2.drm.DrmInitData;
...
import app.judo.shaded.exoplayer2.video.VideoRendererEventListener.EventDispatcher;
 ...
 *       payload should be one of the integer scaling modes in {@link Renderer.VideoScalingMode}. Note that
...
public class MediaCodecVideoRenderer extends MediaCodecRenderer {
  ...
  public MediaCodecVideoRenderer(
    ...
    scalingMode = Renderer.VIDEO_SCALING_MODE_DEFAULT;
  ...
  protected int supportsFormat(MediaCodecSelector mediaCodecSelector, Format format)
    ...
    if (!MimeTypes.isVideo(mimeType)) {
      return RendererCapabilities.create(RendererCapabilities.FORMAT_UNSUPPORTED_TYPE);
    }
    ...
    if (decoderInfos.isEmpty()) {
      return RendererCapabilities.create(RendererCapabilities.FORMAT_UNSUPPORTED_SUBTYPE);
    }
    if (!supportsFormatDrm(format)) {
      return RendererCapabilities.create(RendererCapabilities.FORMAT_UNSUPPORTED_DRM);
    }
  ...
  public void handleMessage(int messageType, @Nullable Object message) throws ExoPlaybackException {
    if (messageType == Renderer.MSG_SET_SURFACE) {
      setSurface((Surface) message);
    } else if (messageType == Renderer.MSG_SET_SCALING_MODE) {
      scalingMode = (Integer) message;
      MediaCodec codec = getCodec();
      if (codec != null) {
        codec.setVideoScalingMode(scalingMode);
      }
    } else if (messageType == Renderer.MSG_SET_VIDEO_FRAME_METADATA_LISTENER) {
  ...
  private void updateSurfaceFrameRate(boolean isNewSurface) {
    ...
    boolean shouldSetFrameRate = getState() == Renderer.STATE_STARTED && currentFrameRate != Format.NO_VALUE;
